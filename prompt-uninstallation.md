...
- Uninstall Vector: `brew uninstall vector` and optionally `brew untap vectordotdev/brew`. The uninstall leaves behind `/opt/homebrew/etc/vector` (bundled example configs) and an empty `/opt/homebrew/var/log/vector` directory — `rm -rf /opt/homebrew/etc/vector` and `rmdir /opt/homebrew/var/log/vector` for a clean removal.
